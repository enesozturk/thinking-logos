...
- **`working` is the only state that becomes open paths.** Every other form is a solid or a surface — orb, cube, globe, body. Rings read as machinery in a way a ball never does, which is what this state should say. They are great circles of one sphere with axes from the Fibonacci lattice, not random inclinations at differing radii: the second gives a scatter of unrelated ellipses, the first an armillary sphere the eye reads as solid. The couriers launch in order rather than independently — independent phases look like interference, a wave travelling around the rings looks like a mechanism.
