...
<ThinkingLogo logo={art} state="working" />     {/* logo → a ringed planet → logo */}
<ThinkingLogo logo={art} state="waiting" />     {/* logo → a bellows of stacked rings → logo */}
<ThinkingLogo logo={art} state="generating" />  {/* logo → a crystal forming out of noise → logo */}
```
...
- **`working` needs a body at the centre.** It used to have the mark's own dots wander around its edges, which was a nice detail and not an orbit — nothing was going round anything. A planet supplies the thing being orbited, and every ring particle shares one angular rate, which is what makes the motion read as governed rather than as drift.
- **`generating` is structure emerging from disorder.** It was first drawn as a flat canvas, on the reasoning that generation produces a picture — the right metaphor and the wrong object, since every other state is a solid turning in space and a flat panel read as a chart that had wandered in. A crystal keeps the metaphor and rejoins the family: crystal growth is where order-from-noise happens in nature. Its surface is a Fibonacci direction normalised by its L1 norm, the exact counterpart of the L∞ norm that gives `solving` its cube, so an octahedron falls out of one line. Colour carries it too — a dot still condensing is neutral grey and takes the brand tint as it lands.
