...
<ThinkingLogo logo={art} state="generating" />  {/* logo → a canvas resolving out of noise → logo */}
```
...
- **`generating` is the only flat form.** Every other state is a solid turning in space; generation is a picture arriving, and it arrives the way diffusion models actually make one — out of noise, resolving rather than being drawn. Making it a sphere would be decorating the wrong idea. Colour carries it too: a dot still resolving is neutral grey and takes the brand tint as it lands, so the canvas visibly develops.
