...
<ThinkingLogo logo={art} state="working" />     {/* carriers orbit between seats, gaining colour */}
...
- **Subtlety does not survive being made small.** `working` used to jitter every dot on a noise field and `breathing` pulsed the mark a few percent in place. Both are pleasant at 140px and completely invisible at 24, which is where a loading indicator actually ships. What survives is contrast and silhouette, not displacement — so `working` holds the mark back and lifts carriers out of it that go grey in the air and take the brand colour back on as they seat, and `breathing` becomes a body whose own outline moves.
