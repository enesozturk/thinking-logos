...
<ThinkingLogo logo={art} state="working" />     {/* logo → tilted orbit rings, with couriers → logo */}
...
- **The mark never simply sits.** It arrives, breathes once, and leaves — nothing in the cycle is flat except the working form's dwell, which is the one pause that is actually wanted. An earlier version held the logo still and then switched a highlight on as a separate event; every such join is a moment where the motion visibly stops and something else starts.
...
- **Subtlety does not survive being made small.** `working` used to jitter every dot on a noise field and `breathing` pulsed the mark a few percent in place. Both are pleasant at 140px and completely invisible at 24, which is where a loading indicator actually ships. What survives is contrast and silhouette, not displacement — so `working` becomes rings with bright couriers crossing between them, and `breathing` becomes a body whose own outline moves.
- **`working` is the only state that becomes open paths.** Every other form is a solid or a surface — orb, cube, globe, body. Rings read as machinery in a way a ball never does, which is what this state should say. The couriers only work because they have somewhere to be: crossing from one ring to another, a courier is legible for its whole life, where one arriving from off-frame spends most of its motion in dead space.
