...
All four transforming states share one cycle: the working form dwells and does its work, the mark snaps in, breathes once, and dissolves again. `dwell` is the knob worth reaching for — how long the cube gets to solve, the globe to scan, the body to pulse. The morph itself is `easeInOutExpo`, the same curve as CSS `cubic-bezier(0.87, 0, 0.13, 1)`: near-still at both ends, very fast through the middle.
